        ...
        <div className="">
            <div className="flex flex-col">
                ...
                {data.length === 0 && status !== "loading" && (
                    <div className="h-[202px] flex flex-row justify-center items-center bg-gray-100 text-gray-300 rounded-xl">
                    ...
                    return (
                        ...
                        >
                            <div className="flex flex-row text-sm mb-4 justify-between">
                                <div className="text-[#f7931a] mr-2">
                                    <CopyText copyText={item.txid}>{shortTransactionHash(item.txid, 10)}</CopyText>
                                </div>
                                {item.status.confirmed ? (
                                    <div className="text-neutral-500">
                                        {dayjs(item.status.block_time * 1000).fromNow()}
                                    </div>
                                ) : (
                            ...
                            <div className="flex flex-row flex-col sm:flex-row sm:items-start items-center">
                                <div className="sm:flex-1 w-full">
                                    {item.vin.map(input => {
                                        return (
                                            <div
                                                key={input.txid}
                                                className="flex flex-row text-xs justify-between mb-2"
                                            >
                                ...
                                <div className="w-[50px] text-center text-2xl rotate-90 sm:rotate-0">
                                ...
                                <div className="sm:flex-1 w-full">
                                    {item.vout
                                        ...
                                        .map((input, index) => {
                                            return (
                                                <div key={index} className="flex flex-row text-xs justify-between mb-2">
                                                    <div className="text-[#6CD7B2]">
                                    ...
                                    {item.vout.length > 5 && (
                                        <div className="text-xs">
                                            ... total {item.vout.length} {lang["items"]}
                                        </div>
                                    )}
                                </div>
                            </div>
