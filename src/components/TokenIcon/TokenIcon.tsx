...
function IconUnknown(props: {str: string; size: number}) {
    ...
    return (
        <div
            ...
            className={`rounded-full text-white flex flex-row justify-center items-center`}
        >
...
export default function TokenIcon({
    ...
    const tokenIcon =
        ...
        'data:image/svg+xml;base64,' + new Identicon(getStrHash(symbol), options as any).toString();
    const chainIcon = chain ? ChainIcons[chain] : undefined;

    return (
        <div className={`${className} relative mr-3`} style={{width: size + 'px', height: size + 'px'}}>
            {!!TokenIcons[symbol.toUpperCase()] ? (
                <img
                    src={tokenIcon}
            ...
            {chainIcon && (
                <img
                    ...
                    style={{
                        marginRight: (size / 8) * -3 + 'px',
                        marginTop: -((size / 8) * -3) + 'px',
                    }}
                    width={(size / 4) * 3}
                    height={(size / 4) * 3}
                    alt=""
