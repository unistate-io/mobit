...
export const getEvmExplorerLink = (network: string) => {
    const base = EvmExplorer[network as keyof typeof EvmExplorer]
    if (!base) {
        return ''
    }
    return `${base}`
...
export default function ListEvmTokenHistory({
                                        data,
                                        status,
                                        address,
                                        network
                                    }: { data: EvmTokenTransaction[], status: string, address?: string, network: string }) {
    ...
    return <div className="">
        <div className="flex flex-col">
...

            {
                data.map((item, index) => {
                    return <Link
                        target="blank"
                        to={`${getEvmExplorerLink(network)}/tx/${item.hash}`} key={index}
                        className="bg-stone-50 rounded p-4 mt-3">
                        <div className="flex flex-row text-sm mb-4 justify-between">
                            <div
                                className="text-[#6CD7B2] mr-2">
                                <CopyText copyText={item.hash}>
                                    {shortTransactionHash(item.hash, 10)}
                                </CopyText>
                            </div>
                            <div className="text-neutral-500">{dayjs(item.metadata.blockTimestamp).fromNow()}</div>
                        </div>
                        <div className="flex flex-col sm:flex-row items-center justify-start">
                            <div className="">
                                <CopyText copyText={item.from}>
                                        {shortTransactionHash(item.from, 10)}
                                </CopyText>
                            </div>
                            <div className="w-[50px] text-center text-2xl rotate-90 sm:rotate-0"><i className="uil-arrow-right" /></div>
                            <div className="">
                                <CopyText copyText={item.to}>
                                        {shortTransactionHash(item.to, 10)}
                                </CopyText>
                            </div>
                        </div>
                        <div className="font-semibold">{item.value} {item.asset}</div>
                    </Link>
                })
            }
        </div>

        {
            ...
            <Link
                to={`${getEvmExplorerLink(network)}/address/${address}`}
