...
export default function DialogSwap({
  children,
  className,
  sellToken,
  onOpen,
  ...
  const swapFromOptions = useMemo<SelectOption[]>(() => {
    ...
    if (!!swapForm.selectedY) {
      const list = pools.filter(
        pool =>
          pool.assetX.typeHash === swapForm.selectedY!.typeHash || pool.assetY.typeHash === swapForm.selectedY!.typeHash
      );
      list.forEach(pool => {
  ...
  const swapToOptions = useMemo<SelectOption[]>(() => {
    ...
    if (!!swapForm.selectedX) {
      const list = pools.filter(
        pool =>
          pool.assetX.typeHash === swapForm.selectedX!.typeHash || pool.assetY.typeHash === swapForm.selectedX!.typeHash
      );
      list.forEach(pool => {
                  ...
                  <div className="flex flex-col py-5 border rounded-2xl mb-4">
                    <div className="flex flex-row items-center justify-between mb-2 px-5">
                      ...
                      <div className="text-sm">
                        ...
                        <span onClick={handleSetMax} className="font-semibold cursor-pointer text-[#6cd7b2] ml-2">
                          MAX
                        </span>
                      </div>
                    </div>
                            ...
                            return (
                              <div className="flex flex-row items-center flex-1 text-nowrap">
                                {!!swapForm.selectedX!.logo ? (
                                  <img className="w-5 h-5 rounded-full mr-3" src={swapForm.selectedX!.logo} alt="" />
                                ) : (
                          ...
                          getOptionLabel={opt => {
                            return (
                              <div className="flex flex-row items-center">
                                {!!opt.token.logo ? (
                                  <img className="w-5 h-5 rounded-full mr-3" src={opt.token.logo} alt="" />
                                ) : (
                            ...
                            return (
                              <div className="flex flex-row items-center flex-1 text-nowrap">
                                {!!swapForm.selectedY!.logo ? (
                                  <img className="w-5 h-5 rounded-full mr-3" src={swapForm.selectedY!.logo} alt="" />
                                ) : (
                  ...
                  <div className="flex flex-row items-center justify-between px-6 mb-4">
                    {!!swapForm.pool && swapForm.amountX ? (
                      <>
                        <div className="text-sm">
                          {lang['Fee']}{' '}
                          <span>{`(${((swapForm.pool as any).poolInfo.feeRate / 10000).toFixed(3)}%)`}</span>
                        </div>
                ...
                <div className="my-4 p-3 bg-gray-100 rounded-lg">
                  ...
                  <div className="flex flex-row flex-nowrap justify-between text-sm mb-2">
                    ...
                    <div className="font-semibold flex flex-row">
                      <CopyText copyText={txHash || ''}>{txHash ? shortTransactionHash(txHash) : '--'}</CopyText>
                    </div>
                  </div>
                </div>
