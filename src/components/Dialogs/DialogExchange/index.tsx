...
export const DialogExchange = ({className, value, options = [], onChange}: Props) => {
    ...
    const filteredOptions = useMemo(() => {
        return options.filter(
            option =>
                option.token?.symbol.toLowerCase().includes(searchWord.toLowerCase()) ||
                option.token?.name?.toLowerCase().includes(searchWord.toLowerCase())
        );
    }, [options, searchWord]);
    ...
    return (
        <Dialog.Root open={open} onOpenChange={setOpen}>
            <Dialog.Trigger
                className={`${className} flex h-[56px] items-center  w-full bg-gray-100 px-3 py-3 rounded-xl justify-between`}
            >
                {value ? (
                    <div className="flex flex-row items-center gap-[12px]">
                        <div className="relative">
                            {value.logo ? (
                                <img src={value.logo} className="w-[48px] h-[48px]" />
                            ) : (
                                <TokenIcon symbol={value.symbol} size={48} className="mr-[0px]" />
                            )}
                            <img
                                src="/images/logo_ckb.png"
                                className="w-[22px] h-[22px] rounded-full absolute bottom-0 right-[-10px]"
                            />
                        </div>
            ...
            <Dialog.Portal>
                ...
                >
                    ...
                    <Input
                        className="h-[50px]"
                        startIcon={<i className="uil-search text-black" />}
                            ...
                            {filteredOptions.map(({token}, i) => {
                                return (
                                    ...
                                    >
                                        <div className="relative ">
                                            {token.logo ? (
                                                <img src={token.logo} className="w-[30px] h-[30px]" />
                                            ) : (
                                                <TokenIcon symbol={token.symbol} size={30} className="mr-[0px]" />
                                            )}
                                            <img
                                                src={'/images/logo_ckb.png'}
                                                className="w-[12px] h-[12px] absolute rounded-full bottom-[-2px] right-[-4px]"
                                            />
                                        </div>
