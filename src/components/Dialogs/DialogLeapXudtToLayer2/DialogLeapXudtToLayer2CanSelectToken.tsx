...
export default function DialogLeapXudtToLayer2CanSelectToken({
  children,
  className,
}: {
  ...
  const [btctxHash, setBtCtxHash] = useState<string>('');
  ...
  useEffect(() => {
    if (!!xudtsBalance && xudtsBalance.length > 0) {
      setToken(xudtsBalance[0]);
    }
  }, [xudtsBalance]);
            ...
            {step === 1 && (
              <>
                ...
                <div className="font-semibold mb-10">
                  ...
                  <Select
                    disabled={xudtsBalanceStatus === 'loading' || !tokenList.length}
                    className="bg-[#F8F9F8] rounded-lg px-2 py-3"
                    placeholder={'Select Token...'}
                    value={token?.symbol}
                    onValueChange={value => {
                      const target = xudtsBalance.find(token => token.symbol === value);
                ...
                <div className="font-semibold mb-10">
                  <div className="mb-2 flex-row flex items-center justify-between">
                    ...
                    <div className="font-normal">
                      ...
                      {token ? toDisplay(balance, token!.decimal, true) : '--'}
                    </div>
                  </div>
                  <Input
                ...
                <Button btntype={'primary'} onClick={checkAndBuild} loading={xudtsBalanceStatus === 'loading' || busy}>
                  {lang['Continue']}
                </Button>
              </>
            )}
            ...
            {step === 3 && (
              <>
                ...
                <div className="text-center text-sm">
                  {lang['The leap action will be completed after this transaction has been confirmed by more than']}
                  <b>6 {lang['blocks']}</b>
                </div>
                ...
                <div className="my-4 p-3 bg-gray-100 rounded-lg">
                  ...
                  <div className="flex flex-row flex-nowrap justify-between text-sm mb-2">
                    ...
                    <div className="font-semibold">
                      {formData.to ? <ProfileAddresses addresses={[formData.to]} defaultAddress={formData.to} /> : '--'}
                    </div>
                  </div>
                  <div className="flex flex-row flex-nowrap justify-between text-sm mb-2">
