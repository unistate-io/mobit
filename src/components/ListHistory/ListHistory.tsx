        ...
        <div>
            <div className="flex flex-col px-3">
                ...
                {data.length === 0 && status !== "loading" && (
                    <div className="h-[120px] flex flex-row justify-center items-center bg-gray-100 text-gray-300 rounded-xl">
                ...
                {showData.map((item, index) => {
                    return (
                        <Link
                            target="blank"
                            to={`${config.explorer}/transaction/${item.attributes.transaction_hash}`}
                            key={item.id}
                            className="bg-stone-50 rounded p-4 mt-3"
                        >
                            <div className="flex  flex-row text-xs justify-between mb-3">
                                <div className="flex flex-row">
                                    <div className="text-[#6CD7B2] mr-2">
                                        {shortTransactionHash(item.attributes.transaction_hash)}
                                    </div>
                                    <div className="text-neutral-500">
                                        {dayjs(item.attributes.created_at.replace(/-/g, "/")).fromNow()}
                                    </div>
                                </div>
                                {item.attributes.rgb_txid && (
                                    <div
                                        className="text-neutral-500 text-sx text-gray-500 px-2 px-1 rounded ml-4"
                                        style={{
                                            background:
                                                "linear-gradient(90.24deg,#ffd176 .23%,#ffdb81 6.7%,#84ffcb 99.82%)"
                                        }}
                                    >
                                        RGB++
                                    </div>
                                ...
                                calculateTotalAmount(item, addresses).map(res => {
                                    const color = res.delta.includes("+")
                                        ...
                                        : res.delta.includes("-")
                                          ...
                                          : "text-neutral-500"
                                    return (
                                        <div key={res.symbol} className="flex flex-row justify-between mt-1 text-xs">
...
function calculateTotalAmountRgbpp(data: TransactionHistory, internalAddress: string) {
    const inputs = data.attributes.display_inputs.filter(
        input => input.rgb_info && input.rgb_info!.address === internalAddress
    )
    const outputs = data.attributes.display_outputs.filter(
        output => output.rgb_info && output.rgb_info!.address === internalAddress
    )
    const total = [...inputs, ...outputs]
