...
export default function useGetXudtCell(
    tokenInfo?: TokenInfoWithAddress,
    addresses?: string[],
) {
    const { config, network, signer } = useContext(CKBContext);
    const [data, setData] = useState<Cell[]>([]);
    const [status, setStatus] = useState<"loading" | "error" | "complete">(
        "loading",
    );
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const scriptConfig = network === "mainnet"
                ? lumosConfig.MAINNET
                : lumosConfig.TESTNET;
    ...
    const createBurnXudtCellTx = async (burnAmount: bigint) => {
        if (
            !tokenInfo || !addresses || !addresses.length ||
            burnAmount === BigInt(0)
        ) return null;
        const ckbHelper = new CkbHelper(network === "mainnet");
    ...
    const createTransferXudtCellTx = async (receivers: {
        toAddress: string;
        ...
        let tx = await createTransferXudtTransaction({
            xudtArgs: tokenInfo.address.script_args.replace("\\", "0"),
            ckbAddress: addresses[0],
            receivers,
            collector: ckbHelper.collector,
