...
export default function Profile() {
  ...
  const tokenData = useMemo(() => {
    if (tokensStatus === 'loading' || tokensStatus === 'error') {
      ...
      return layer1Btc ? [ckbData!, ...xudtData, layer1Btc, ...layer1Xudt] : [ckbData!, ...xudtData, ...layer1Xudt];
    }
        ...
        <div className="flex justify-between flex-col lg:flex-row">
          ...
          <div className={`lg:max-w-[380px] flex-1 mt-4 ${currtab !== 'Activity' ? 'block' : 'hidden'}`}>
            <div className="shadow rounded-lg bg-white py-4">
              ...
              {activeTab === 'ckb' && (
                <ListHistory maxShow={5} addresses={queryAddress} data={historyData} status={historyDataStatus} />
              )}
              ...
              {activeTab === 'rgbpp' && btcAddress && (
                <ListHistory internalAddress={internalAddress!} data={rgbppHistory} status={rgbppHistoryStatus} />
              )}
            </div>
          </div>
        </div>
