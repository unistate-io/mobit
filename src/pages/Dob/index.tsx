      ...
      <div className="md:w-[320px] w-full shadow rounded-lg overflow-hidden shrink-0">
        <div className="w-full h-[320px] relative">
          <img
            ...
            src={image || 'https://explorer.nervos.org/images/spore_placeholder.svg'}
            alt=""
          />
        </div>
        <div className="p-4">
          ...
          {status !== 'loading' && !!data && (
            <>
              ...
              <div className="flex flex-row justify-between text-sm">{des}</div>
            </>
          )}
        ...
        {status !== 'loading' && data && (
          <>
            ...
            <div className="text-sm mb-6">
              ...
              <div className="flex flex-row items-center text-sm font-semibold break-all">{data.content_type}</div>
            </div>

            <div className="text-sm mb-6">
              <div className="text-sm mb-3">{lang['Token ID']}</div>
              <div className="flex flex-row items-center text-sm font-semibold break-all" title={`0x${tokenid}`}>
            ...
            <div className="text-sm mb-6">
              ...
              >
                <CopyText copyText={data.owner_address}>{shortTransactionHash(data.owner_address, 10)}</CopyText>
              </div>

              {isOwner && chain === 'btc' && (
                <div className="flex flex-row items-center text-sm font-semibold break-all">
                  <CopyText copyText={internalAddress!}>{shortTransactionHash(internalAddress!, 10)}</CopyText>
                </div>
              )}
            </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-center text-sm font-semibold break-all whitespace-nowrap overflow-hidden overflow-ellipsis">
                              {traits.traits[0].String || traits.traits[0].Number}
                            </div>
                          </div>
                        );
                      })}
