...
export default function MarketPage() {
    ...
    return (
        <div className="max-w-[--page-with] mx-auto px-3 mt-4 md:mt-10 mb-10">
            <div
                className="py-9 mb-4 px-4 flex flex-col justify-center items-center shadow rounded-lg overflow-hidden"
                style={{
                    background: 'url("/images/market_image.png") center bottom no-repeat',
                    backgroundSize: "100% 200px"
                }}
            >
                <img src="/images/logo.png" className="w-[114px] h-[32px] mb-3" alt="" />
                <div
                    className="font-semibold text-xl text-center mb-4"
                    dangerouslySetInnerHTML={{
                        __html: lang["Effortlessly and securely transfer assets between Bitcoin and CKB"]
                    }}
                ></div>
                <div className="text-[#7B7C7B] text-sm mb-4">
                    {lang["Enjoying a seamless cross-chain experience with RGB++ Leap functionality!"]}
                </div>
                {!address && (
                    <button
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td className="py-4 text-right text-lg">
                                            <DisPlayChange change={item.change_24h} />
                                        </td>
                                    </tr>
                                )
                            })}
            ...
            {data.length !== 0 &&
                ...
                data?.map((item, index) => {
                    return (
                        <div className="flex flex-col p-4 shadow rounded-lg bg-white mb-3" key={index}>
                            ...
                            <div className="flex flex-row items-center mb-4">
                                <div className="mr-[30px] min-w-[35%]">
                                    ...
                                    <div className="text-lg font-semibold flex">${item.price}</div>
                                </div>

                                <div>
                                    <div className="text-sm">{lang["MarketCap"]}</div>
                                    <div className="text-lg font-semibold">
                                        ${toDisplay(item.market_cap + "", 0, true, 0)}
                                    </div>
                                </div>
                            </div>
                            <div className="flex flex-row items-center">
