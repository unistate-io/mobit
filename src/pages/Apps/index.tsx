...
export default function AppsPage() {
  ...
  return (
    <div className="max-w-[--page-with] mx-auto mt-4 md:mt-10 mb-10">
      <div className="font-semibold text-2xl mb:4 md:mb-9 px-3 ">CKB {lang['Apps']}</div>
      <div className="flex flex-row flex-wrap w-full">
        {data.map((item, index) => {
          return (
            <div className="basis-full sm:basis-1/2 md:basis-1/3 px-3 pb-3" key={index}>
              <Link to={item.url} target="_blank" className="shadow p-4 md:p-6 rounded-xl block">
                <div className="flex flex-row items-center">
                  ...
                  <div className="ml-6 text-xl font-semibold overflow-hidden line-clamp-2 line-clamp-ellipsis">
                    {item.name}
                  </div>
                </div>
                <div className="my-2 h-8 overflow-hidden line-clamp-2 line-clamp-ellipsis text-xs text-gray-400">
                  {item.description[langType] || item.description['en']}
                </div>
